...
export class CreateColor {
    ...
    public async getResult(): Promise<Color | null> {

        const dataColor: ColorInterface = {
            id_color: null,
        ...
        const allColor: Array<Color> | null = await manager.getColor();

        if (allColor.length != 0) {
            // vérif
            allColor.forEach((c: Color) => {
                if (JSON.stringify(c.light) === JSON.stringify(newColor.light)) {
        ...
        if (result.affectedRows == 1 && result.insertId > 0) {
            // Color inséré
            return this._ctx.body = newColor;
        } else {
            // Echec insert
